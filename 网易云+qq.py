...
class MyFrame1(threading.Thread, wx.Frame):
    ...
    def __init__(self, threadID, name, counter):
        ...
        self.url_text = wx.TextCtrl(self, wx.ID_ANY, wx.EmptyString, wx.DefaultPosition, wx.Size(200, -1), 0)
        ...
        # 新增按钮：手动输入歌单列表
        self.manual_input_button = wx.Button(self, wx.ID_ANY, u"手动输入歌单列表", wx.DefaultPosition, wx.DefaultSize,
                                             0)
        bSizer5.Add(self.manual_input_button, 0, wx.ALL, 5)
        ...
        self.manual_input_button.Bind(wx.EVT_BUTTON, self.onManualInput)  # 新增绑定
    ...
    # 新增：手动输入歌单列表的事件处理
    def onManualInput(self, event):
        dlg = wx.TextEntryDialog(self, "请输入歌单列表的HTML代码", "手动输入歌单列表", "",
                                 style=wx.OK | wx.CANCEL | wx.TE_MULTILINE)

        if dlg.ShowModal() == wx.ID_OK:
            html = dlg.GetValue()
            songs = self.parseManualSongList(html)
            if songs:
                ...
                if not os.path.exists(self.dest_folder):
                    os.makedirs(self.dest_folder)
                wx.CallAfter(self.output_text.AppendText, f"解析到 {len(songs)} 首歌曲，开始使用改进版下载...\n")
                self.start()
            else:
                wx.MessageBox("未解析到任何歌曲信息", "提示")
        dlg.Destroy()

    # 新增：解析手动输入的HTML，提取歌曲名称和歌手信息
    def parseManualSongList(self, html):
        songs = []
        # 提取每个<li>元素
        li_pattern = re.compile(r'<li.*?>(.*?)</li>', re.S)
        li_items = li_pattern.findall(html)
        for item in li_items:
            # 提取歌曲名称：从 <span class="songlist__songname_txt"> 下的 <a title="...">
            name_match = re.search(r'<span class="songlist__songname_txt">.*?<a[^>]+title="([^"]+)"', item, re.S)
            if not name_match:
                continue
            song_name = name_match.group(1).strip()
            # 提取歌手：从 <div class="songlist__artist"> 下的 <a title="...">
            artist_match = re.search(r'<div class="songlist__artist">.*?<a[^>]+title="([^"]+)"', item, re.S)
            artist = artist_match.group(1).strip() if artist_match else ""
            songs.append({'name': song_name, 'artist': artist})
        return songs
            ...
            for future in as_completed(future_to_song):
                ...
                try:
                    ...
                    wx.CallAfter(self.output_text.AppendText,
                                 "***** " + song.get('name', '未知歌曲') + " 下载出错: " + str(exc) + "\n")
                progress_value += 1
    ...
    def process_song(self, song):
        ...
        try:
            # 原版下载链接（适用于通过URL获取歌单的情况）
            original_url = 'http://music.163.com/song/media/outer/url?id=' + str(song.get('id', '')) + '.mp3'
            wx.CallAfter(self.output_text.AppendText, "尝试原版下载: " + song_name + "\n")
            if song.get('id') and self.saveFile(original_url, dest_path):
                if self.is_valid_audio_file(dest_path):
